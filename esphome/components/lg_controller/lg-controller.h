...
class LgController final : public climate::Climate, public uart::UARTDevice, public Component {
    ...
    // Last received 0xB4/0xD0 message.
    ...
    void configure_capabilities() {
        ...
        device_modes.insert(climate::CLIMATE_MODE_OFF); // Power Off
        device_modes.insert(climate::CLIMATE_MODE_FAN_ONLY); // Power On
        
        climate::ClimateFanModeMask fan_modes;
        fan_modes.insert(climate::CLIMATE_FAN_LOW); // Low
        fan_modes.insert(climate::CLIMATE_FAN_MEDIUM); // Medium
        fan_modes.insert(climate::CLIMATE_FAN_HIGH); // High
        fan_modes.insert(climate::CLIMATE_FAN_AUTO); // Auto
        
        climate::ClimatePresetMask preset_modes;
        preset_modes.insert(climate::CLIMATE_PRESET_NONE); // Add Off
        preset_modes.insert(climate::CLIMATE_PRESET_BOOST); // Add Fast
        preset_modes.insert(climate::CLIMATE_PRESET_ECO); // Add eSave
...
public:
    ...
    void setup() override {
        ...
        ESP_LOGI(TAG, "Starting LG ERV Controller - Slave mode: %s", slave_ ? "enabled" : "disabled");
    }
